...
> {
  ...
  private async createTokenListProvider(chainId: ChainId, tokenCache: NodeJSCache<Token>): Promise<ITokenListProvider> {
    try {
      // Fetch official Uniswap token list
      const DEFAULT_TOKEN_LIST_URL = 'https://gateway.ipfs.io/ipns/tokens.uniswap.org'
      
      console.log(`Fetching token list for chain ${chainId} from ${DEFAULT_TOKEN_LIST_URL}`)
      ...
      const tokenListData = await response.json() as TokenList
      
      // Filter tokens for the specific chain
      const chainTokens = tokenListData.tokens.filter(token => token.chainId === chainId)
      
      console.log(`Found ${chainTokens.length} tokens for chain ${chainId}`)
      
      const filteredTokenList: TokenList = {
        ...tokenListData,
        tokens: chainTokens,
        name: `${tokenListData.name} - Chain ${chainId}`,
      }
